...
class EmptyASR:
    def transcribe(self, pcm, sample_rate):
        return []  # silence / no speech recognised

    def close(self):
        pass


def test_empty_transcript_sends_no_speech(tmp_path, monkeypatch):
    audio_dir = tmp_path / "audio"
    audio_dir.mkdir()
    monkeypatch.setattr(settings, "audio_dir", audio_dir)
    app = FastAPI()
    app.state.asr_engine = EmptyASR()
    app.state.note_generator = StubNoteGen()
    app.include_router(ws_router.router)
    messages = _run_session(TestClient(app), "no-speech")
    assert any(m["type"] == "error" and m["code"] == "NO_SPEECH" for m in messages), messages
    assert not any(m["type"] == "note_done" for m in messages)
...
def test_asr_failure_sends_error_and_deletes_wav(tmp_path, monkeypatch):
    audio_dir = tmp_path / "audio"
    audio_dir.mkdir()
    monkeypatch.setattr(settings, "audio_dir", audio_dir)
    app = FastAPI()
    app.state.asr_engine = FailingASR()
    app.state.note_generator = StubNoteGen()
    app.include_router(ws_router.router)
    messages = _run_session(TestClient(app), "asr-fail")
    assert any(m["type"] == "error" and m["code"] == "ASR_FAILED" for m in messages), messages
    assert not any(m["type"] == "note_done" for m in messages)
    assert list(audio_dir.glob("*.wav")) == []
